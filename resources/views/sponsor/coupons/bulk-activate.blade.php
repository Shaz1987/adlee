<x-layouts.app-layout title="Activate Coupon" is-sidebar-open="true">
    <div
        ...
        <a href="{{ $isBookletCoupons ? route('sponsors.booklets.show', $bookletId) : route('sponsors.coupons.index') }}"
                            ...
                            <div class="relative w-full max-w-2xl origin-bottom rounded-lg bg-white pb-4 transition-all duration-300 dark:bg-navy-700"
                                ...
                                <div class="p-4 sm:p-5">
                                    ...
                                    <div x-show="showCommemorationForm">
                                        <form action="{{ route('sponsors.coupons.activate.bulk.store') }}"
                                            method="post">
                                            @csrf
                                            <input type="hidden" name="coupons" value="{{ json_encode($coupons) }}">
                                            <div class="space-y-5">
                                                <!---- language --->
                                                <div class="">
                                    ...
                                    <div x-show="!showCommemorationForm" class="text-center mt-3">
                                        <p class="text-sm font-normal text-slate-600 mb-4">Are you sure to activate
                                            ...
                                            charged for total of ${{ $totalAmount }} for {{ count($coupons) }} coupons
                                            after redemption them by Ad Space Owner. </p>
                                        <form action="{{ route('sponsors.coupons.activate.bulk.store') }}"
                                            method="post">
                                            @csrf
                                            <input type="hidden" name="coupons" value="{{ json_encode($coupons) }}">
