...
class CouponPayoutController extends Controller
{
    ...
    public function bulk(Request $request)
    {
        ...
        $data['isBookletCoupons'] = Coupon::where('uuid', $request->coupons)->whereNotNull('booklet_id')->exists() ? true : false;
        $data['bookletId'] = Coupon::where('uuid', $request->coupons)->whereNotNull('booklet_id')->first()->booklet_id;
    ...
    private function sendPayment($coupon, $amount, $stripe)
    {
        ...
        if ($plan) {
            ...
            if (isset($payment->latest_charge)) {
                ...
                Notification::send($coupon->redeem, new PaymentNotification($coupon->redeem->name, $coupon->number, $remainingAmount));
